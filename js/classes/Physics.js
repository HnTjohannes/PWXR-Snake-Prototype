class Physics {
  updatePlayerMovement(state, canvas, dt) {
    ...
    state.head.x += (target.x - state.head.x) * CONSTANTS.PHYSICS.PLAYER_LERP;
    state.head.y += (target.y - state.head.y) * CONSTANTS.PHYSICS.PLAYER_LERP;
  }

  updateTrail(state) {
    const last = state.trail[state.trail.length - 1];
    
    if (!last || this.getDistance(state.head, last) > CONSTANTS.PHYSICS.MIN_DISTANCE) {
  ...
  detectLoop(trail) {
    if (trail.length <= CONSTANTS.PHYSICS.MIN_LOOP_LENGTH) return null;
    
    const head = trail[trail.length - 1];
    const minGap = CONSTANTS.PHYSICS.MIN_GAP;
    const closeDist2 = CONSTANTS.PHYSICS.CLOSE_DISTANCE_SQUARED;
    let loopStart = -1;
